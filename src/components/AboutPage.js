import React from "react";
import "../AboutPage.css";
import Axitec from "../images/Axitec copy.png";
import { useState,useEffect } from "react";
import jinko from "../images/jinko (1).png";
import Novasys from "../images/Novasys copy.png";
import solex from "../images/solex copy.png";
import Navbar from "./Navbar";
import Footer from "./footer";
import teamimg from "../images/teamimage1.jpg"
import about_img from "../images/about_us_image.png"

const About = () => {

useEffect(()=>{
  const videobtn = document.getElementById("videolink")
  videobtn.addEventListener('click', ()=>{
    window.open("https://youtu.be/HhjHYkPQ8F0");
  })
})


  return (
    <>
      <Navbar />       
      <div className="mainpscon">

      <div className="row row1">
        <div className="col-md-6 firstCol">
          <h3 className="aboutHeading">About Us</h3>

          <div className="button1">
            <a href="">
              Home<i className="far fa-angle-right"></i>
              <span>About Us</span>
            </a>
          </div>
        </div>
        <div className="col-md-6 secondCol">
          <img
            className="image image-fluid"
            src={about_img}
            alt=""
          />
        </div>
      </div>

      <div
        className=" container-fluid d-flex"
        style={{
          width: "85%",
          marginBottom: "30px",
          marginTop: "30px",
          justifyContent: "space-evenly",
          backgroundColor: "#282438",
          borderRadius:"20px"
        }}
      >
        <img
          className="img-fluid"
          src={jinko}
          alt="jinko"
          style={{ width: "14%" }}
        />
        <img
          className="img-fluid"
          src={solex}
          alt="solex"
          style={{ width: "14%" }}
        />
        <img
          className="img-fluid"
          src={Novasys}
          alt="Novasys"
          style={{ width: "14%" }}
        />
        <img
          className="img-fluid"
          src={Axitec}
          alt="Axitec"
          style={{ width: "14%" }}
        />
      </div>
      <div
        class="pscon12 about-3_area about-3space about-3plr about-3_plr-2"
        style={{}}
      >
        <div className="container-fluid">
          <div className="row align-items-center justify-content-center">
            <div className=" col-md-5 mb-70 mainimagesection text-end">
              {/* <img
                className="mainimage"
                src="https://ordainit.com/react/sorex/static/media/about-6.2cc2faf11fdd960f84c7.png"
                alt=""
              /> */}
              <img src={teamimg} alt="teamimg" width={"100%"} />
              <div className="videoplay" id="videolink" style={{cursor:'pointer'}} >
                <div className="playbuttondiv">
                  <div className="playlink">
                    <i className="fas fa-play playicon img-fluid"></i>
                    </div>
                </div>
                <div className="watchvideo"  >
                 <a  className="watchvideolink"  >
                    <span>Watch Video</span>
                  </a> 
                  
                </div>
              </div>
            </div>
            <div className="col-xl-5  col-lg-6 col-md-12 mb-70  div2">
              <div className="maincontent">
                <div class="about-3__section-title pb-10">
                  <h4 class="section-blue-subtitle char-anim"> About OneKlick Techno Renewable</h4>
                  <h6 class="section-title1 section-title char-anim">
                   Authorized Distributor of  <i>Jinko Solar </i>, <i>Solex</i>, <i>Novasys Green</i> & <i>Axitec</i><br />
                  </h6>
                </div>
                {/* <div class="about-3__section-title pb-10">
                  <h4 class="section-blue-subtitle char-anim"> About </h4>
                  <h6 class="section-title1 section-title char-anim">
                  Oneklick Techno Renewable <br />
                  </h6>
                </div> */}
                <div className="para">
                  <div className="para1 pb-7">
                    <p className="paracontent" style={{textTransform:'capitalize'}}>
                    Oneklick Techno Renewable is a pan India authorized distributor of solar modules from leading brands like Jinko Solar, Solex, Axitec, and Novasys.  We provide our customers with three key features:
                    </p>
                  </div>
                  <div className="points">
                    <ul>
                      <li>
                        <i className="fal fa-check-square"></i>
                        <span style={{textTransform:'capitalize'}}>
                        quality products, operational efficiency  and total commitment.
                        </span>
                      </li>
                      <li>
                        <i className="fal fa-check-square"></i>
                        <span style={{textTransform:'capitalize'}}>Our customer satisfaction reflects everything about us and makes us the best solar panel distributor company in India.</span>
                      </li>
                      {/* <li>
                        <i className="far fa-angle-double-right"></i>
                        <span>
                          Business tailored it design, management &amp;
                          services.{" "}
                        </span>
                      </li> */}
                    </ul>
                  </div>
                  <div className="More">
                    <a className="Morecontent" href="#">
                      <span className="spanbutton">About More</span>
                      <b></b>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="row ps-cardrow">
        <div className="col-md-2 pscol">
          <div className="pscardimg pscardimg1">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>

          <h4 className="h4ps">Quality</h4>
        </div>
        <div className="col-md-2 pscol">
          <div className="pscardimg pscardimg2">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>
          <h4 className="h4ps">Trust</h4>
        </div>
        <div className="col-md-2 pscol">
          <div className="pscardimg pscardimg3">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>
          <h4 className="h4ps">Affordable</h4>
        </div>
        <div className="col-md-2 pscol">
          <div className="pscardimg pscardimg4">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>
          <h4 className="h4ps">Sustainability</h4>
        </div>
        <div className="col-md-2 pscol">
          <div className="pscardimg pscardimg5">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>
          <h4 className="h4ps">Open Communication</h4>
        </div>
      </div>
      <div class="team_area teamspace-2 team_plr grey-bg">
        <div className="container-fluid team">
          <div className="row align-items-end justify-content-center">
            <div className="col-xl-12 col-lg-6 col-md-9 col-sm-10">
              <div className="feature__section-box text-center">
                <h4 className="section-blue-subtitle title-anim wow animate__fadeInUp">
                  Meet Team Member
                </h4>
                <h3 className="section-title title-anim wow animate__fadeInUp">
                  Industries we Meet This <br />
                  Team Meamber.
                </h3>
              </div>
            </div>
          </div>
          <div className="row psrow d-flex justify-content-center">
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
     
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
  
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
          </div>
          <div className="row psrow">
      
            
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
   
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
      
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
 
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
            <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
              {/* <div className="team__item text-center psteam">
    <div className="team__main-thumb">
        <img src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg" alt="Barasa Perats"/>

</div>
<div class="ps-overlay">
    <div class="text">Hello World</div>
  </div>

</div> */}
              <div class="container23">
                <img
                  src="https://ordainit.com/react/sorex/static/media/team-1.c732656045256c3d48ca.jpg"
                  alt="Avatar"
                  class="image"
                />
                <div class="ps-overlay">
                  <div class="text"></div>
                </div>
              </div>
              <div
                style={{ textAlign: "center" }}
                className="team__author-info"
              >
                <h5 className="team__author-name">
                  <a href="/react/sorex/about-me">Barasa Perats</a>
                </h5>
                <span>Founder</span>
              </div>
            </div>
         
          </div>
        </div>
      </div>

      <div className="work-3_area work-3inner-wrapper work-3plr work-3_space">
        <div className="container-fluid" style={{ marginTop: "120px" }}>
          <div className="row row-cols-xl-3 row-cols-md-2 ">
            <div className="col col-6 col-12 wow animate__fadeInUp">
              <div className="work-3__wrapper inner-wrapper-1 mb-30">
                <div className="work-3__item1">
                  <div className="work-3__content  align-items-center mb-15">
                    <div className="work-3__icon icon-color-1">
                      <span>
                        <i class="fa-solid fa-thumbs-up"></i>
                      </span>
                    </div>

                    <div className="work-3__title">
                      <h4 className="work-3__section-title">
                        <a href="#" style={{ lineHeight: "30px" }}>
                          Highest quality products at Cost Effective price
                        </a>
                      </h4>
                    </div>
                  </div>
                  <div className="work-3__right-text">
                    <p>
                      We know that it can be hard to find the right products for
                      your projects from such wide variety of choices,
                      especially when you need quality and price both. That’s
                      why we make sure that we advise you best suited product
                      which are always delivered 100% in time and at an
                      affordable price—and we’re committed to making it happen.
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="col col-6 col-12 wow animate__fadeInUp">
              <div class="work-3__wrapper inner-wrapper-2 mb-30">
                <div class="work-3__item2">
                  <div class="work-3__content  align-items-center mb-15">
                    <div class="work-3__icon icon-color-2">
                      <span>
                        <i class="fa-solid fa-user-group"></i>
                      </span>
                    </div>

                    <div class="work-3__title">
                      <h4 class="work-3__section-title">
                        <a href="#"> Professional and Enthusiastic team</a>
                      </h4>
                    </div>
                  </div>
                  <div class="work-3__right-text">
                    <p>
                      We strive to stay on top of new technologies and
                      techniques so that we can deliver the best possible
                      outcomes. Our team members are always willing to help —
                      from advice on best suited products on your projects to
                      technical support.
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="col col-6 col-12 wow animate__fadeInUp">
              <div class="work-3__wrapper inner-wrapper-3 mb-30">
                <div class="work-3__item3">
                  <div class="work-3__content  align-items-center mb-15">
                    <div class="work-3__icon icon-color-3">
                      <span>
                        <i class="fa-solid fa-user-check"></i>
                      </span>
                    </div>

                    <div class="work-3__title">
                      <h4 class="work-3__section-title">
                        <a href="#">Reliability</a>
                      </h4>
                    </div>
                  </div>
                  <div class="work-3__right-text">
                    <p>
                      OneKlick is committed to quality so we deliver whatever
                      you want, whenever you want and wherever you want. We are
                      available 24/7/365 to answer any questions you may have.
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="col col-6 col-12 wow animate__fadeInUp">
              <div class="work-3__wrapper inner-wrapper-4 mb-30">
                <div class="work-3__item4">
                  <div class="work-3__content  align-items-center mb-15">
                    <div class="work-3__icon icon-color-4">
                      <span>
                        <i class="fa-sharp fa-solid fa-magnifying-glass"></i>
                      </span>
                    </div>

                    <div class="work-3__title">
                      <h4 class="work-3__section-title">
                        <a href="#">Transparency</a>
                      </h4>
                    </div>
                  </div>
                  <div class="work-3__right-text">
                    <p>
                      OneKlick ensures that our customers receive accurate
                      information and all the relevant details that they need in
                      order to find the product they are looking for.
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="col col-6 col-12 wow animate__fadeInUp accord">
              <div class="work-3__wrapper inner-wrapper-5 mb-30">
                <div class="work-3__item5">
                  <div class="work-3__content  align-items-center mb-15">
                    <div class="work-3__icon icon-color-5">
                      <span>
                        <i class="fa-solid fa-signal-bars"></i>
                      </span>
                    </div>

                    <div class="work-3__title">
                      <h4 class="work-3__section-title">
                        <a href="#">Premium Range</a>
                      </h4>
                    </div>
                  </div>
                  <div class="work-3__right-text">
                    <p>
                      From World’s most sold Solar module to German standard
                      practices. We make sure that we deliver you the best.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <Footer/>

      </div>
   
    </>
  );
};

export default About;
